...
This section covers Model A, which includes the outcome variable 1978 earnings (`re78`) and adjusts for a set of covariates: age, education, race indicators (black, hispanic), marital status, lack of high school degree, 1974 and 1975 earnings (`re74`, `re75`), and unemployment status in 1974 and 1975 (`u74`, `u75`). The model is defined by a logistic regression formula for treatment assignment using these covariates. To improve covariate balance and overlap between treated and control groups, various methods were applied and structured into five parts: Matching, Weighting, Trimming, Truncation, and Combined Methods integrating these techniques. From these, the top 5 performing methods were identified based on absolute standardized mean differences and effective sample size, and the resulting datasets were used to estimate the Average Treatment effect on the Treated (ATT). Notably, the estimation incorporated the recently introduced Augmented Inverse Probability Weighting (AIPW) estimator, implemented via the `OutcomeWeights` R package. Subsequently, placebo tests were conducted using 1975 earnings as an outcome to assess potential biases and the validity of unconfoundedness assumptions. Additionally, sensitivity analyses were performed to evaluate the robustness of treatment effect estimates to violations of these assumptions. 
...
dataset_list_cps <- list(
  ...
  #"smr_ate_weight"                              = ldw_cps_plus$smr_ate_weight,
  ## Truncation
  ...
  #"matching_common_range"                       = matching_common_range.cps_plus,
  #"matching_crump"                              = matching_crump.cps_plus,
  #"matching_ps_threshold"                       = matching_ps_threshold.cps_plus,
  #"matching_stuermer"                           = matching_stuermer.cps_plus,
  #"matching_walker"                             = matching_walker.cps_plus,
)
```
...
#top5_datasets.psid_plus <- create_top5_datasets(all_psid_plus, top5_methods.psid_plus)
```
-----
```{r}
all_cps_plus$Method
...
```{r}
#names(all_methods.cps_plus)
```
-----

```{r}
# save them into .RData files
save_top5_individual_files(all_methods.cps_plus, top5_methods.cps_plus, prefix = "model_a_cps")
save_top5_individual_files(all_methods.psid_plus, top5_methods.psid_plus, prefix = "model_a_psid")
```
...
out4 <- estimate_all(ldw_cps_trim, "re78", "treat", covar)
out5 <- estimate_all(ldw_psid_trim, "re78", "treat", covar)
```
...
# combine all results
all_outs <- list(out1, out2, out3, out4, out5, out6, out7, out8, out9, out10, out11, out12, out13, out14, out15)
```
...
# save results
save_att_panels(all_outs, base_titles, top5_methods.cps_plus, top5_methods.psid_plus, band, est, "model_a")
```
...
Across the CPS1-PLUS samples (figures 4–8), all estimators yield positive ATT estimates that tightly cluster around the experimental benchmark (red dashed line).

In PSID1-PLUS samples (figures 9–13), all estimators produce positive ATT estimates close to the experimental benchmark, with the exception of `propensity score matching` applied to the `card` sample, which produces a slightly negative ATT estimate. 

The PSID1-PLUS samples also display marginally higher mean standard errors (in total - not method-wise) and exhibit a broader range of effect estimates (in total - not method-wise) compared to the CPS1-PLUS samples.
...
Within the CPS1-PLUS dataset, the `exact` matching sample (column 4) and the `mahvars` matching sample (column 6) provide positive estimates near the experimental benchmark. In contrast, the `profile` (column 5), `optimal_pair` (column 7), and `card` (column 8) samples generate estimates slightly further from the benchmark, though still relatively close across most estimators.

Regarding the PSID1-PLUS dataset, the `mahvars` (column 10) and `optimal_pair` (column 12) samples deliver estimates very close to the experimental benchmark, while the profile sample (column 9) exhibits a moderate deviation. The `k3` (column 11) and `card` (column 13) samples, however, produce estimates that deviate more substantially from the benchmark.

Across all observational samples based on CPS1-PLUS and PSID1-PLUS datasets (columns 4 to 13), the standard errors tend to be higher than in the experimental sample, reflecting increased uncertainty associated with these estimations.
```{r}
# save results 
...
```
Specifically, with LDW-CPS1, CATT estimates span from $-5,745.166 to $6,956.62, contrasting with the CATT estimated from experimental data which ranges from $-71.17303 to $3,813.05. Employing LDW-PSID generates CATT estimates ranging from $-8,145.318 to $4,567.563. With the datasets obtained from the top-ranked methods applied to the CPS1-PLUS and PSID1-PLUS datasets, the CATT estimates exhibit varying ranges that often lie within or close to the experimental benchmark, with some methods such as `exact` and `profile` producing more positive minimum and maximum values, while others show wider spreads including substantial negative minima, reflecting heterogeneity in treatment effect estimates across methods and samples.
...
out4_pl <- estimate_all(ldw_cps_trim, Y_pl, "treat", covar)
out5_pl <- estimate_all(ldw_psid_trim, Y_pl, "treat", covar)
```
...
out6_pl <- out_cps_pl[[1]]
out7_pl <- out_cps_pl[[2]]
out8_pl <- out_cps_pl[[3]]
out9_pl <- out_cps_pl[[4]]
out10_pl <- out_cps_pl[[5]]
out11_pl <- out_psid_pl[[1]]
out12_pl <- out_psid_pl[[2]]
out13_pl <- out_psid_pl[[3]]
out14_pl <- out_psid_pl[[4]]
out15_pl <- out_psid_pl[[5]]
```

```{r, out.width='100%', fig.asp=1, fig.cap="FIGURE 13. Placebo Test Model B: '75 Earnings as the Outcome"}
#| code-fold: show
# collect all placebo results 
all_outs_pl <- list(out1_pl, out2_pl, out3_pl, out4_pl, out5_pl, out6_pl, out7_pl, out8_pl, out9_pl, out10_pl, out11_pl, out12_pl, out13_pl, out14_pl, out15_pl)
...
# save results
save_att_panels(all_outs, base_titles, top5_methods.cps_plus, top5_methods.psid_plus, band, est, "model_a_placebo")
```
...
The placebo analysis reveals that the experimental benchmark (first row and first column) for 1975 earnings is near zero and statistically insignificant, while all estimators applied to the observational datasets, CPS1 and PSID1, yield large, negative ATT estimates.

The top-ranked samples of CPS1-PLUS and PSID1-PLUS produce diverse results. For the CPS1-PLUS dataset, the `exact` matching sample (4) always gives zero ATT estimates, indicating no bias; however, its practical utility is limited by an extremely small effective sample size. The `profile` (5) sample, `mahvars` (6) and `optimal_pair` (7) samples yield positive values close to experimental benchmark, whereas the `card` sample (8) yields negative ATT estimates with occasional positive exceptions across the different estimators. 

For the PSID1-PLUS dataset, the `profile` sample (9), `k3` sample (11), and `card` sample (13) produce negative ATT estimates, with some positive exceptions across the different estimators. In contrast the `mahvars` sample (10) and `optimal_pair` sample (12) yield positive ATT estimates closer to the experimental benchmark, where the former sample has some negative ATT estimates across the different estimators.

Across all estimators and observational top-ranked samples (5–13), with the exception of the exact matching sample (4), the placebo analysis reveals substantial bias and deviation from the true effect, demonstrating the inherent limitations of confounding adjustment using observational data.  

...
# comb.cps.plus <- list(
# ps_threshold_match = ldw_cps_plus.trim.match)
#
...
```{r, message=FALSE, warning=FALSE}
# compute SMD
# smd_all_comb_meth.cps <- compute_abs_smd_comb(comb.cps.plus, "treat", covar) 
#smd_all_comb_meth.psid <- compute_abs_smd_comb(comb.psid.plus, "treat", covar)
```

#### ESS
```{r, message=FALSE, warning=FALSE}
# compute ESS
# ess_all_comb_meth.cps <- compute_ess_comb(comb.cps.plus, "treat", covar) 
# ess_all_comb_meth.psid <- compute_ess_comb(comb.psid.plus, "treat", covar)
# ess_all_comb_meth.psid
```

#### Visuals
```{r, message=FALSE, warning=FALSE, fig.cap='FIGUREA7.'}
# visualize overlap
# plot_comb_overlap(comb_meth_cps = comb.cps.plus, comb_meth_psid = comb.psid.plus, treat = "treat", covar = covar, prefix_cps = "LDW-CPS1-PLUS", prefix_psid = "LDW-PSID1-PLUS")
```
...
# "ps_threshold_match" = ldw_psid_plus.trim.match,
  "ps_threshold" = ldw_psid.trim,
  "common_range" = ldw_psid.common,
  "stuermer" = ldw_psid.stuermer,
  "walker" = ldw_psid.walker,
...
out4 <- estimate_all(ldw_cps.trim, "re78", "treat", covar) 
out5 <- estimate_all(ldw_psid.trim, "re78", "treat", covar)
out6 <- estimate_all(ldw_cps_plus.trim, "re78", "treat", covar) # similar to Imbens & Xu (2024)
out7 <- estimate_all(ldw_psid_plus.trim, "re78", "treat", covar) # similar to Imbens & Xu (2024)
...
```{r, message=FALSE, warning=FALSE}
# save results
save_att_panels(all_outs, plot_titles, band, est, "ldw_model_a")
```
...
qte.ldw_cps_plus.trim <- est_qte(Y, treat, covar, data = ldw_cps_plus.trim) # similar to Imbens & Xu (2024)
qte.ldw_psid_plus.trim <- est_qte(Y, treat, covar, data = ldw_cps_plus.trim) # similar to Imbens & Xu (2024)
```
...
qte.ldw_psid_plus.trim0 <- est_qte(Y, treat, NULL, data = ldw_cps_plus.trim)
...
out4_pl <- estimate_all(ldw_cps.trim, Y_pl, "treat", covar_pl) 
out5_pl <- estimate_all(ldw_psid.trim, Y_pl, "treat", covar_pl)
out6_pl <- estimate_all(ldw_cps_plus.trim, Y_pl, "treat", covar_pl) # similar to Imbens & Xu (2024)
out7_pl <- estimate_all(ldw_cps_plus.trim, Y_pl, "treat", covar_pl) # similar to Imbens & Xu (2024)
```

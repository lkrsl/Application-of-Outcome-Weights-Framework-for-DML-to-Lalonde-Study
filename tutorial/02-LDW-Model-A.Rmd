...
<div class="callout-note">
In each plot the black dots represent the ATT point estimates. The red dashed line indicates the respective benchmark and the light red band the 95% confidence interval. 
</div>
...
The LDW-CPS-1 sample produces ATT estimates that closely align with the benchmark with an exception for one estimator. Its top-ranked subsamples produce ATT estimates that are consistently positive and cluster reasonably close around the benchmark, except for the `cem_stuermer` subsample that exhibits somewhat larger deviations from the benchmark and increased standard errors. 
...
The QTETs estimated from the LDW-CPS-1 sample (B) and their top-ranked subsamples (D, E and H), except for `k2_crump` (F) and `cem_stuermer` (G), corresponds well with the true QTET, although the estimates are often underpowered. The QTET estimates from the trimmed and matched version of LDW-CPS-1-PLUS (N) also correspond well with the true QTET. 

The QTET estimates from the LDW-PSID-1 (C) and their top-ranked subsamples (I-M) show clear biases and noticeably wider confidence bands when compared to the experimental benchmark, which clusters near zero, indicating greater estimation uncertainty. Only the top-ranked subsamples `mahvars_walker` (L) and `k2_walker` (K) produce QTET estimates that are slightly closer to the true QTET. The QTET estimates from the trimmed and matched version of LDW-PSID-1-PLUS (N) also exhibit clear biases.
```{r, message=FALSE, warning=FALSE}
# list results
plots_ldw <- list(
...
For LDW-CPS-1 (B), most ATT estimates are negative. For LDW-CPS-1 subsamples (D-H), ATT estimates remain largely negative with only a few exceptions but show reasonable improvement in ATT estimates towards the benchmark and overly decreased standard errors compared to the LDW-CPS-1 (B) sample, still indicating persistent bias. For LDW-PSID-1 subsamples (I-M) a similar pattern results compared to the LDW-PSID-1 sample. 
...
Overall, across most observational samples and estimators, the placebo test produces ATT estimates that are substantially biased and far from the experimental benchmark. This highlights the persistent challenge in recovering unbiased ATT estimates outside of experimental data.
```{r, message=FALSE, warning=FALSE}
# save results 
...
For the top-ranked subsample `cem_stuermer` (G) of the LDW-CPS-1 sample as well as for the LDW-PSID-1 sample (C) and all its top-ranked subsamples (I-M) the estimated treatment effects are sensitive to unmeasured confounding with more pronounced swings, as evidenced by sizable shifts in the estimates.
...
### Correlation 
To evaluate potential collinearity between `re74` and `re75`, we compute their correlation. To determine whether `re74` may act as a confounder in treatment effect estimation, we report its correlation with both the treatment assignment variable (`treat`) and the outcome (`re78`).
```{r, message=FALSE, warning=FALSE}
...
datatable(cor_table, caption = "Correlations of re74 with treatment and outcome across samples",
          options = list(scrollX = TRUE, paging = FALSE, pageLength = -1, searching = FALSE))
```
The tabulated results demonstrate that while 1974 earnings (`re74`) are only weakly correlated with treatment assignment across all samples, ranging from approximately −0.32 to 0.18, they exhibit moderately strong positive correlations with the outcome variable, ranging from approximately −0.03 to 0.70, suggesting that this pretreatment covariate constitutes a meaningful confounder of post-program earnings, but not a driver of treatment assignment. Further, 1974 earnings exhibits substantial correlation with 1975 earnings, implying considerable collinearity between these two lagged covariates and suggesting that the informational gain from including both may be limited.
...
To further assess the risk of confounding,
we examine the distributional balance of `re74` and, for comparison, of `re75` by plotting their density distributions for treated and control units. As model B intentionally omits `re74`, the analysis directly informs whether any covariate imbalance may induce bias in estimated treatment effects.
...
for (i in seq_along(all_datasets)) {
  tryCatch({
    print(
      bal.plot(treat ~ re75, data = all_datasets[[i]], which = "both") +
...
for (i in seq_along(all_datasets)) {
  tryCatch({
    print(
      bal.plot(treat ~ re74, data = all_datasets[[i]], which = "both") +
        ggplot2::ggtitle(all_plot_titles[i])
    )
  }, error = function(e) {
    cat(sprintf("Could not plot %s: %s\n", all_plot_titles[i], e$message))
...
# save results
save_balance(all_datasets, all_plot_titles, balance_var = "re75", prefix = "ldw_model_a_balance_panels_75")
save_balance(all_datasets, all_plot_titles, balance_var = "re74", prefix = "ldw_model_a_balance_panels_74")
```
...
Thus, considering the pronounced collinearity with 1975 earnings, the moderate correlation with the outcome, and the negligible balance concerns, 1974 earnings may be excluded from the covariate set. However, whether its inclusion or exclusion materially affects the treatment effect estimates warrants validation. Accordingly, model B is specified without 1974 earnings and the complete analysis redone. 

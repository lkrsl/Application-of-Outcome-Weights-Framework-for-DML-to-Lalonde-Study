...
We merge the experimental controls from LDW-Experimental into LDW-CPS1 and LDW-PSID1 to augment the control groups with additional observations, following a similar approach as by Imbens & Xu (2024).
```{r}
#| code-fold: show
...
```{r, class.source = "fold-show", fig.cap='FIGURE A1. SubfigureA:LDW. SubfigureB:LDW-CPS1. SubfigureC:LDW-PSID1.',out.width='100%', fig.asp=0.5}
#| code-fold: show
# assess overlap
...
```{r, class.source = "fold-show", fig.cap='FIGURE A2. SubfigureD:LDW-CPS1-PLUS. SubfigureE:LDW-PSID1-PLUS.', out.width='80%', fig.asp=1, fig.align='center'}
#| code-fold: show
# assess overlap
...
```{r, class.source = "fold-show", fig.cap='FIGURE A3. SubfigureA:LDW-CPS1. SubfigureB:LDW-CPS1-PLUS. SubfigureC:LDW-PSID1. SubfigureD:LDW-PSID1-PLUS.', out.width='80%', fig.asp=1, fig.align='center'}
...
for (wcol in weight_columns) {
  if (wcol %in% names(ldw_psid_plus)) {
    ...
    cat("Column", wcol, "not found in ldw_psid.plus\n")
  }
}
```
...
#### Visuals
```{r, class.source = "fold-show", fig.cap='FIGURE A4.'}
#| code-fold: show
...
#### Visuals
```{r, class.source = "fold-show", fig.cap='FIGURE A5.'}
#| code-fold: show
# visualize covariate balance
...
#### Visuals
```{r, class.source = "fold-show", fig.cap='FIGURE A6.'}
#| code-fold: show
# visualize covariate balance
...
#### Visuals
```{r, class.source = "fold-show", fig.cap='FIGURE A7.'}
#| code-fold: show
# visualize overlap
...
```{r, class.source = "fold-show", fig.cap='FIGURE A8.'}
#| code-fold: show
# visualize covariate balance
...
```{r, message=FALSE, warning=FALSE, class.source = "fold-show", fig.cap='FIGURE A10.'} 
# visualize covariate balance
...
We utilize the `estimate_all()` and `plot_coef()` functions as defined by Imbens & Xu (2024).
```{r, message=FALSE, warning=FALSE}
# estimate ATT
...
```{r, out.width='100%', fig.asp=1, fig.cap="FIGURE A11. ATT Estimates Model A Given Unconfoundedness using LDW Samples"}
# plot results
band <- out1[1, 3:4]
est  <- out1[1, 1]
...
Figure (A) presents the benchmark from the experimental sample (LDW-Experimental), serving as a reference for bias and variance assessment of observational estimators. Figures (B) and C3) show results for the observational samples, LDW-CPS1 and LDW-PSID1, while figures (D) and (E) present those for the trimmed versions, replicating the tutorial results of Imbens & Xu (2024). Figures (F) through (J) display results for CPS1-derived subsamples constructed with the top-ranked methods. Analogously, figures (K) through (O) summarize results for the corresponding PSID1-derived subsamples under parallel rules.

Across the LDW-CPS1 and its top-ranked subsamples, all estimators generally yield ATT estimates that closely cluster around the experimental benchmark. However, there are slightly larger deviations for the `overlap_crump` and `overlap_pair` subsamples. While most estimates are positive, the  `overlap_crump` and `optimal_pair` subsamples include a few negative ATT estimates, and all ATT estimates for the `mahvars` subsample are negative.

In contrast, PSID1-based subsamples frequently exhibit greater dispersion and substantially higher standard errors compared to LDW-CPS1 samples. Most ATT estimates are negatively aligned, indicating increased methodological uncertainty in these samples. Positive ATT estimates emerge only for the `overlap_crump` subsample.
```{r, warning=FALSE}
...
Then, we employ a modified version of the function `plot_catt()` from Imbens & Xu (2024) to visualize the results by plotting the estimated CATTs for all samples at the covariate values of each treated unit against their corresponding experimental benchmarks. 
...
:::
```{r, out.width='100%', fig.asp=1, fig.cap="FIGURE A12. CATT Estimates Model A using LDW Data"}
#| code-fold: show
...
Specifically, with LDW-CPS1, CATT estimates span from $-4,979.99 to $7,201.81, contrasting with the CATT estimated from experimental data which ranges from $-189.40	to $3,835.95, with a mean CATT of $1,713.26. The LDW-PSID1 data shows an even broader CATT range, spanning from $-8,643.02 to $4,903.22, and a notably lower mean estimate of approximately $820.77.

Among the trimmed and top-ranked CPS1 subsamples, CATT ranges vary substantially. Specific methods such as `card` or `overlap_crump` produce minimum CATTs that are considerably negative, but produce positive mean estimates. Methods like`ps_threshold`, and `optimal_pair` also deliver positive mean CATT estimates, though with smaller negative minimums, while `mahvars` is the only method among these to produce a negative mean CATT. Importantly, across all methods, the mean CATT estimates are far away from experimental mean estimate. 

The CATT estimates for the PSID1 trimmed and top-ranked subsamples reveal substantially decreased mean values and wider extremes compared to their CPS1 counterparts, reflecting greater difficulties in producing reliable treatment effect estimates.

This variation in range and averages across methods and samples reflects substantial heterogeneity in treatment effect estimation, indicating that while some samples or methods improve alignment with experimental benchmarks, others introduce considerable discrepancies and spread in estimated heterogeneous effects.
...
```{r, out.width='100%', fig.asp=1, fig.cap="FIGURE A13. QTET Estimates Model A using LDW Data: Experimental vs. Nonexperimental"}
#| code-fold: show
# CPS
...
# save results
save_qtet()
save_qte_top(qte.top5_cps_plus, qte.top5_cps_plus0, qte.ldw_cps, plot_titles, main_start = 6, ylim = c(-25000, 15000), prefix = "model_a_top_cps")
save_qte_top(qte.top5_psid_plus, qte.top5_psid_plus0, qte.ldw_psid, plot_titles, main_start = 11, ylim = c(-25000, 15000), prefix = "model_a_top_psid")
```

These figures display QTET estimates derived from both the LDW experimental and various observational samples. The QTETs estimated from the original and trimmed LDW-CPS1 sample (B and D) corresponds well with the true QTET, although the estimates are often underpowered. The QTET estimates from the original and trimmed LDW-PSID1 sample (C and E) show clear biases when compared to the experimental benchmark, which clusters near zero. Among the top-ranked subsamples, CPS1-based QTETs (F - J) continue to track the true experimental effect well, whereas PSID1-based QTETs (K - O) display increased bias and noticeably wider confidence bands, indicating greater estimation uncertainty.
...
```{r, out.width='100%', fig.asp=1, fig.cap="FIGURE A14. Outcome Weights Model A using LDW Data"}
#| code-fold: show
...
::: callout-tip
The outcome weights are not obtained directly from the ATT point estimates due to computational design of other methods. Instead, the ATT is separately estimated for each dataset following the approach developed by Knaus and Pfleiderer (2024).
:::

The evaluation reveals that across each datasets the estimated outcome weights sum to one within the treated group and minus one within the untreated group, resulting in an overall sum of zero.
...
```{r, out.width='100%', fig.asp=1, fig.cap="FIGURE A15. Placebo Test Model B: '75 Earnings as the Outcome"}
#| code-fold: show
...
For LDW-CPS1, most ATT estimates are negative. The `overlap_crump` sample shows a modest improvement toward the experimental benchmark. The trimmed samples (D) and (J), which use similar approaches, further improve CPS1 (B) results. The `optimal_pair` and `card` samples provide even greater improvement. The `mahvars` (G) sample delivers ATT estimates closest to the experimental benchmark and the only positive ATT estimates observed among CPS1 samples.

For LDW-PSID1, the sample (O) applying the `overlap_common_range` method shows no improvement compared to the PSID1 sample (C). The `overlap_crump` sample yields ATT estimates closer to the experimental benchmark, with the `card` and `mahvars` samples producing estimates even closer to the experimental benchmark. The trimmed samples (E) and (N), which use similar approaches, achieve the closest alignment with the benchmark.
...
```{r, out.width='100%', fig.asp=1, fig.cap="FIGURE A16. Sensitivity Analyses Model A"}
#| code-fold: show

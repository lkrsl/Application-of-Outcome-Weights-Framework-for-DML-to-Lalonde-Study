...
```{r, warning=FALSE}
#| code-fold: show
# get outcome weights for ATT
res_ow_att <- lapply(all_datasets, function(d) aipw_ow(d, Y, treat, covar))
ow_att <- lapply(results_ow, function(res) get_outcome_weights(res$dml_fit))
```

```{r, warning=FALSE}
#| code-fold: show
# get ow for each CATT estimator
res_ow_att <- lapply(all_datasets, function(d) aipw_ow(d, Y, treat, covar))
ow_att <- lapply(results_ow, function(res) get_outcome_weights(res$dml_fit))
```
...
```{r, warning=FALSE}
#| code-fold: show
# get ow for each QTET estimator
res_ow_att <- lapply(all_datasets, function(d) aipw_ow(d, Y, treat, covar))
ow_att <- lapply(results_ow, function(res) get_outcome_weights(res$dml_fit))
```
...
for (i in seq_along(outcome_weights)) {
  # Extract weights for ATT
  weights <- outcome_weights[[i]]$omega["AIPW-ATT", ]

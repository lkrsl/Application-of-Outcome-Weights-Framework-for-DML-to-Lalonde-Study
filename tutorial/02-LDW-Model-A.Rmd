...
Next, we augment the control groups in LDW-CPS1 and LDW-PSID1 by merging them with the experimental controls from LDW-Experimental. This approach follows the methodology laid out by @imbens2024.
```{r, message=FALSE, warning=FALSE}
...
Initial covariate balance checks reveal the degree to which treatment and control groups differ on observed characteristics. We employ visual summaries using `love.plot()`, which depicts standardized mean differences across covariates before and after adjustment.
...
Neither LDW-CPS1-PLUS nor LDW-PSID1-PLUS shows consistently improved covariate balance compared with their non-plus counterparts based on raw standardized mean differences. Although some specific covariates improve slightly, most show increased imbalance.
...
m.out.cps.mahvars <- matchit(model, data = ldw_cps, method = "nearest", distance = "logit", caliper = 0.2, mahvars = ~ age + education + re74 + re75, replace = FALSE)
m.out.psid.mahvars <- matchit(model, data = ldw_psid, method = "nearest", distance = "logit", caliper = 0.2, mahvars = ~ age + education + re74 + re75, replace = FALSE)
```
...
The purpose of weighting is to reweight the units in order to create a synthetic population in which the distributions of covariates is balanced between treatment and control groups. In the following, several weighting methods as outlined by @Stuermer_2021 are applied.
...
ldw_cps_plus_trim <- ps_trim(ldw_cps_plus.ps, threshold = 0.9)
ldw_psid_plus_trim <- ps_trim(ldw_psid_plus.ps, threshold = 0.8)

# exclude experimental controls, subset trimmed data appropriately
ldw_cps_plus.trim.match <- subset(ldw_cps_plus_trim, sample %in% c(1,3) & ps_assoverlap)
ldw_psid_plus.trim.match <- subset(ldw_psid_plus_trim, sample %in% c(1,4) & ps_assoverlap)
...
# list all weights
all_weight.cps <- list(
  ipw = ldw_cps$ipw_weight,
  opt = ldw_cps$opt_weight,
  cbps = ldw_cps$cbps_weight,
  ebal = ldw_cps$ebal_weight
)

all_weight.psid <- list(
  ipw = ldw_psid$ipw_weight,
  opt = ldw_psid$opt_weight,
  cbps = ldw_psid$cbps_weight,
  ebal = ldw_psid$ebal_weight
)
```
...
#### Visuals
```{r, message=FALSE, warning=FALSE, fig.cap='FIGUREA7.'}
# visualize overlap
...
save_comb_loveplots(comb_meth_cps = comb.cps, comb_meth_psid = comb.psid, treat = "treat", covar = covar, prefix = "model_a", prefix_cps = "ldw_cps1", prefix_psid = "ldw_psid1")
```

## Identifying best methods

To identify the top five methods for each observational dataset, we first combine for each dataset all results of the SMD and ESS into a single data frame. This allows for a comprehensive comparison across all methods.
```{r, message=FALSE, warning=FALSE}
# combine all results
...
- `smd_score` rescales the SMD to a 0-1 range, where a lower value leads to a higher score
...
```{r, message=FALSE, warning=FALSE}
# save results 
save_csv(top5_methods.cps_plus, "ldw_model_a_cps1_top5_methods")
save_csv(top5_methods.psid_plus, "ldw_model_a_psid1_top5_methods")
```

For estimation, we match the selected top method names back to their corresponding datasets, objects or vectors and construct corresponding datasets. 
```{r, message=FALSE, warning=FALSE}
...
save_top5_individual_files(dataset_list_cps, top5_methods.cps, prefix = "ldw_model_a_cps1")
save_top5_individual_files(dataset_list_psid, top5_methods.psid, prefix = "ldw_model_a_psid1")
```

## Estimating
### Average treatment effect on the treated (ATT)

Next, we estimate the average treatment effect on the treated (ATT) using both the LDW-Experimental sample, the non-plus observational datasets and the newly constructed top five ranked samples for each observational dataset, LDW-CPS1 and LDW-PSID1, that achieved highest composite score results. 
...
```{r, message=FALSE, warning=FALSE, out.width='100%', fig.asp=1, fig.cap="FIGUREA9. ATT Estimates Model A Given Unconfoundedness using LDW Samples"}
# plot results
band <- out1[1, 3:4]
est  <- out1[1, 1]
...
The above figures show the ATT estimates and their 95% confidence intervals for fifteen samples: LDW-Experimental, LDW-CPS1, LDW-PSID1, trimmed versions of the LDW-CPS1 and LDW-PSID1 samples (similar to @imbens2024) and a series of top-ranked subsamples of both LDW-CPS1 and LDW-PSID1 based on various matching, weighting, truncation and trimming criteria.
...
Across the LDW-CPS1 dataset and its top-ranked subsamples, all estimators generally produce ATT estimates that closely cluster around the experimental benchmark. However, the `adap_weight_trunc_smr_weight` sample exhibit somewhat larger deviations. While most estimates are positive, the `overlap_crump` and `optimal_pair` subsample results include a few negative ATT estimates, and all ATT estimates for the `mahvars` subsample are consistently negative.
...
knitr::kable(result_mat, caption = "ATT Estimates and SEs", booktabs = TRUE) %>% kable_styling(full_width = TRUE) 
```
...
```{r, message=FALSE, warning=FALSE, out.width='100%', fig.asp=1, fig.cap="FIGUREA10. CATT Estimates Model A using LDW Data"}
...
```{r, message=FALSE, warning=FALSE, out.width='100%', fig.asp=1, fig.cap="FIGUREA11. QTET Estimates Model A using LDW Data: Experimental vs. Nonexperimental"}
par(mfrow = c(2,2))
par(cex.main = 0.8)
...
save_qte_top(qte.top5_cps, qte.top5_cps0, qte.ldw_cps, plot_titles, main_start = 8, ylim = c(-25000, 15000), prefix = "ldw_model_a_top")
save_qte_top(qte.top5_psid, qte.top5_psid0, qte.ldw_psid, plot_titles, main_start = 13, ylim = c(-25000, 15000), prefix = "ldw_model_a_top")
```
...
```{r, message=FALSE, warning=FALSE, out.width='100%', fig.asp=1, fig.cap="FIGUREA12. Outcome Weights Model A using LDW Data"}
par(mfrow = c(2,2))
par(cex.main = 0.8)
...
```{r, message=FALSE, warning=FALSE, out.width='100%', fig.asp=1, fig.cap="FIGUREA13. Placebo Test Model B: '75 Earnings as the Outcome"}
...
```{r, message=FALSE, warning=FALSE}
# save results
save_att_panels(all_outs.pl, plot_titles, band_pl, est_pl, "ldw_model_a_placebo")
```
...
```{r, warning=FALSE, message=FALSE, out.width='100%', fig.asp=1, fig.cap="FIGUREA14. Sensitivity Analyses Model A"}
par(mfrow = c(2,2))
par(cex.main = 0.8)
...
```{r, message=FALSE, warning=FALSE}
# save results
save_sensitivity_plots(filtered_datasets_sens, Y, treat, covar, bm, plot_titles, "ldw_model_a")
```

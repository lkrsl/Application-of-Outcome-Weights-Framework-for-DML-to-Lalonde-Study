...
m.out.cps.nearest <- matchit(model, data = ldw_cps, method = "nearest", distance = "glm", link = "logit", replace = TRUE)
m.out.psid.nearest <- matchit(model, data = ldw_psid, method = "nearest", distance = "glm", link = "logit", replace = TRUE)
```
...
m.out.cps.k2 <- matchit(model, data = ldw_cps, method = "nearest", distance = "glm", link = "logit", ratio = k, replace = TRUE)
m.out.psid.k2 <- matchit(model, data = ldw_psid, method = "nearest", distance = "glm", link = "logit", ratio = k, replace = TRUE)
```
...
m.out.cps.k3 <- matchit(model, data = ldw_cps, method = "nearest", distance = "glm", link = "logit", ratio = k, replace = TRUE)
m.out.psid.k3 <- matchit(model, data = ldw_psid, method = "nearest", distance = "glm", link = "logit", ratio = k, replace = TRUE)
```

##### Caliper matching
```{r, message=FALSE, warning=FALSE}
# perform nearest neighbor matching with a caliper of 0.1 on the logistic propensity score 
m.out.cps.caliper <- matchit(model, data = ldw_cps, method = "nearest", distance = "glm", link = "logit", caliper = 0.1, replace = TRUE)
m.out.psid.caliper <- matchit(model, data = ldw_psid, method = "nearest", distance = "glm", link = "logit", caliper = 0.1, replace = TRUE)
```

##### Common support restriction matching
```{r, message=FALSE, warning=FALSE}
# perform nearest neighbor matching with exclusion of units outside common support
m.out.cps.cs <- matchit(model, data = ldw_cps, method = "nearest", distance = "glm", link = "logit", discard = "both", reestimate = TRUE, replace = TRUE)
m.out.psid.cs <- matchit(model, data = ldw_psid, method = "nearest", distance = "glm", link = "logit", discard = "both", reestimate = TRUE, replace = TRUE)
```

##### Mahalanobis distance matching (mahvars) 
```{r, message=FALSE, warning=FALSE}
# perform nearest neighbor matching using mahalanobis distance
m.out.cps.mahvars <- matchit(model, data = ldw_cps, method = "nearest", distance = "mahalanobis", replace = FALSE)
m.out.psid.mahvars <- matchit(model, data = ldw_psid, method = "nearest", distance = "mahalanobis", replace = FALSE)
```

##### Optimal pair matching
```{r, message=FALSE, warning=FALSE}
# perform optimal pair matching that minimizes total within-pair distance on propensity scores
m.out.cps.optimal_pair <- matchit(model, data = ldw_cps, method = "optimal", distance = "glm", link = "logit")
m.out.psid.optimal_pair <- matchit(model, data = ldw_psid, method = "optimal", distance = "glm", link = "logit")
```
...
m.out.cps.optimal_full <- matchit(model, data = ldw_cps, method = "full", distance = "glm", link = "logit")
m.out.psid.optimal_full <- matchit(model, data = ldw_psid, method = "full", distance = "glm", link = "logit")
```
...
m.out.cps.general_full <- matchit(model, data = ldw_cps, method = "quick", distance = "glm", link = "logit")
m.out.psid.general_full <- matchit(model, data = ldw_psid, method = "quick", distance = "glm", link = "logit")
```

##### Genetic matching
```{r, message=FALSE, warning=FALSE}
# perform genetic matching 
m.out.cps.genetic <- matchit(model, data = ldw_cps, method = "genetic", distance = "glm", link = "logit", replace = TRUE, pop.size = 100)
m.out.psid.genetic <- matchit(model, data = ldw_psid, method = "genetic", distance = "glm", link = "logit", replace = TRUE, pop.size = 100)
```
...
m.out.cps.profile <- matchit(model, data = ldw_cps, method = "cardinality", estimand = "ATT", tols = 0.1, ratio = NA, time = 1200)
m.out.psid.profile <- matchit(model, data = ldw_psid, method = "cardinality", estimand = "ATT", tols = 0.1, ratio = NA, time = 1200)
```
...
ldw_cps.ps_fixed <- truncate_ps_fixed(ldw_cps.ps, treat = "treat", ps = "ps_assoverlap", 
                                      lower = 0.05, upper = 0.95)
ldw_psid.ps_fixed <- truncate_ps_fixed(ldw_psid.ps, treat = "treat", ps = "ps_assoverlap", 
                                       lower = 0.05, upper = 0.95)
```
...
ldw_cps.ps_percent <- truncate_ps_percentile(ldw_cps.ps, ps = "ps_assoverlap", 
                                             lower_percentile = 5, upper_percentile = 95)
ldw_psid.ps_percent <- truncate_ps_percentile(ldw_psid.ps, ps = "ps_assoverlap", 
                                             lower_percentile = 5, upper_percentile = 95)
```
...
ldw_cps.trim <- assess_overlap(data = ldw_cps.trim, treat = treat, cov = covar)
ldw_psid.trim <- assess_overlap(data = ldw_psid.trim, treat = treat, cov = covar)
```
...
# re-estimate propensity scores on trimmed data 
ldw_cps.common <- assess_overlap(data = ldw_cps.common, treat = treat, cov = covar)
ldw_psid.common <- assess_overlap(data = ldw_psid.common, treat = treat, cov = covar)
```
...
# re-estimate propensity scores on trimmed data 
ldw_cps.crump <- assess_overlap(data = ldw_cps.crump, treat = treat, cov = covar)
ldw_psid.crump <- assess_overlap(data = ldw_psid.crump, treat = treat, cov = covar)
```
...
# re-estimate propensity scores on trimmed data 
ldw_cps.stuermer <- assess_overlap(data = ldw_cps.stuermer, treat = treat, cov = covar)
ldw_psid.stuermer <- assess_overlap(data = ldw_psid.stuermer, treat = treat, cov = covar)
```
...
# re-estimate propensity scores on trimmed data 
ldw_cps.walker <- assess_overlap(data = ldw_cps.walker, treat = treat, cov = covar)
ldw_psid.walker <- assess_overlap(data = ldw_psid.walker, treat = treat, cov = covar)
```
...
```{r, message=FALSE, warning=FALSE}
# compute absolute SMD
smd_trunc.cps <- compute_abs_smd_trunc(all_trunc.cps, treat = "treat", covar, "ps_assoverlap")
smd_trunc.psid <- compute_abs_smd_trunc(all_trunc.psid, treat = "treat", covar, "ps_assoverlap")
```

#### ESS
```{r, message=FALSE, warning=FALSE}
# compute ESS
ess_trunc.cps <- compute_ess_trunc(all_trunc.cps, treat = "treat", covar, "ps_assoverlap")
ess_trunc.psid <- compute_ess_trunc(all_trunc.psid, treat = "treat", covar, "ps_assoverlap")
```

#### Visuals
```{r, warning=FALSE, message=FALSE, fig.cap='FIGUREA5.'}
# visualize covariate balance
plot_trunc_balance(all_trunc.cps, treat = "treat", covar, "ps_assoverlap", "LDW-CPS1")
plot_trunc_balance(all_trunc.psid, treat = "treat", covar, "ps_assoverlap", "LDW-PSID1")
```
...
```{r, message=FALSE, warning=FALSE}
# compute ESS
ess_trim_match_comb.cps <- compute_ess_trim(trim_match_comb.cps, "treat", covar)
ess_trim_match_comb.psid <- compute_ess_trim(trim_match_comb.psid, "treat", covar)
```
...
# visualize overlap
par(mfrow = c(1, 3))
plot_trim_overlap(trim_match_comb.cps, treat, covar, prefix = "LDW-CPS1-PLUS")
plot_trim_overlap(trim_match_comb.psid, treat, covar, prefix = "LDW-PSID1-PLUS")
```
...
```{r, message=FALSE, warning=FALSE}
# compute ESS
ess_trim_weight_comb.cps <- compute_ess_trim_weight(trim_weight_comb.cps, "treat", covar)
ess_trim_weight_comb.psid <- compute_ess_trim_weight(trim_weight_comb.psid, "treat", covar)
```

#### Visuals
```{r, message=FALSE, warning=FALSE, fig.cap='FIGUREA8.'}
# visualize overlap
plot_comb_overlap(comb_meth_cps = trim_weight_comb.cps, comb_meth_psid = trim_weight_comb.psid, treat = "treat", covar = covar, 
                  prefix_cps = "LDW-CPS1", prefix_psid = "LDW-PSID1")
```

```{r, message=FALSE, warning=FALSE}
# save results
save_comb_hist(comb_meth_cps = trim_weight_comb.cps, comb_meth_psid = trim_weight_comb.psid, treat = "treat", covar = covar, 
               prefix = "model_a", prefix_cps = "ldw_cps1", prefix_psid = "ldw_psid1")
```
...
```{r, message=FALSE, warning=FALSE}
# compute ESS
ess_trunc_weight_comb.cps <- compute_ess_trunc_weight(trunc_weight_comb.cps, "treat", covar, weight_columns)
ess_trunc_weight_comb.psid <- compute_ess_trunc_weight(trunc_weight_comb.psid, "treat", covar, weight_columns)
```

#### Visuals
```{r, warning=FALSE, message=FALSE, fig.cap='FIGUREA5.'}
# visualize covariate balance
plot_trunc_weight_balance(trunc_weight_comb.cps, "treat", covar, weight_columns, "LDW-CPS1")
plot_trunc_weight_balance(trunc_weight_comb.psid, "treat", covar, weight_columns, "LDW-PSID1")
```
...
```{r, message=FALSE, warning=FALSE}
# save results 
save_csv(all_cps, "ldw_model_a_cps1_plus_all_results")
save_csv(all_psid, "ldw_model_a_psid1_plus_all_results")
```
...
```{r, message=FALSE, warning=FALSE}
# save results 
save_csv(top5_methods.cps, "ldw_model_a_cps1_plus_top5_methods")

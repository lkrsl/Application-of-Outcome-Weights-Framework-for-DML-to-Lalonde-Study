...
Next we proceed with the estimation of the ATT using both the LDW-Experimental sample and the newly constructed top 5 samples for each observational dataset, LDW-CPS1-PLUS and LDW-PSID1-PLUS, that had highest score results. 
```{r, message=FALSE, warning=FALSE}
...
Figure (A) presents the benchmark from the experimental sample (LDW-Experimental), serving as a reference for bias and variance assessment of observational estimators.
Figures (B) and C3) show results for the observational samples, LDW-CPS1 and LDW-PSID1, respectively, while figures (D) and (E) present those for the trimmed versions.
Figures (F) through (J) display results for CPS1-PLUS-derived subsamples constructed with the top-ranked methods. Analogously, figures (K) through (O) summarize results for the corresponding PSID1-PLUS-derived subsamples under parallel rules.
...
In contrast, the estimates for LDW-PSID1 and its top-ranked subsamples exhibit greater dispersion and substantially higher standard errors. Most ATT estimates are negatively aligned, indicating increased methodological uncertainty in these samples. Positive ATT estimates emerge only for the `overlap_crump` and `fix_max_value_trunc_overlap_weight` subsamples.
...
Overall, the figures and table jointly demonstrate that trimming subsamples can bring observational estimates closer to the experimental benchmark, particularly for CPS1. Nevertheless, significant estimator-dependent variability and sensitivity to sample construction persist, especially in PSID1. Additionally, ATT estimates from observational samples tend to have larger standard errors compared to the experimental sample, reflecting greater statistical uncertainty in non-experimental causal effect estimation.
```{r}
# save results 
...
Then, we employ a modified version of the `plot_catt()` function from Imbens & Xu (2024) to visualize the results. In the subsequent figures, we plot the estimated CATTs for all samples at the covariate values of each treated unit against their corresponding experimental benchmarks. 
...
```
Specifically, with LDW-CPS1, CATT estimates span from $-5,345.44 to $6,959.96, contrasting with the CATT estimated from experimental data which ranges from $-122.74 to $3,846.87. Employing LDW-PSID generates CATT estimates ranging from $-8,569.46 to $4,552.47. 

For the trimmed and top-ranked subsamples derived from CPS1 and PSID1, the ranges of CATT estimates vary, often approaching or partially overlapping with the experimental benchmark. However, certain methods—such as 'card' or 'mahvars'—produce minima that are substantially more negative, while maxima can be either higher or lower than the experimental upper bound, reflecting substantial heterogeneity in treatment effect estimation across methods and sample constructions.

With the subsamples obtained from the top-ranked methods applied to LDW-CPS1 and LDW-PSID1, the CATT estimates exhibit varying ranges that often lie within or close to the experimental benchmark, with some methods such as `exact` and `profile` producing more positive minimum and maximum values, while others show wider spreads including substantial negative minima, reflecting heterogeneity in treatment effect estimates across methods and samples.

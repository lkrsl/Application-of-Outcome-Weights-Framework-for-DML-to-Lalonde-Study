...
love.plot(lcs_psid_plus, lcs_psid_plus.common, treat, covar, title = "LCS-PSID1-PLUS - common range trimming")
love.plot(lcs_psid_plus, lcs_psid_plus.crump, treat, covar, title = "LCS-PSID1-PLUS - crump trimming")
love.plot(lcs_psid_plus, lcs_psid_plus.stuermer, treat, covar, title = "LCS-PSID1-PLUS - stuermer trimming")
love.plot(lcs_psid_plus, lcs_psid_plus.walker,  treat, covar, title = "LCS-PSID1-PLUS - walker trimming")
```
...
# visualize covariate balance
#plot_comb_love_plots(comb_meth.psid_plus, "treat", covar, prefix_psid = "LCS-PSID1")
```
...
Next we proceed with the estimation of the ATT using both the LCS-Experimental sample and the newly constructed top 5 samples for each observational dataset, LCS-PSID, that had highest score results. 
```{r, message=FALSE, warning=FALSE}
...
#all_qtet <- c(list(qte.lcs, qte.nsw_psid, qte.nsw_psid.trim), qte.top5.cps, qte.top5.psid)
...
To further evaluate the credibility of the unconfoundedness assumption, a placebo analysis is performed by designating the number of children in 1975—a variable not included in LaLonde’s original dataset - as the outcome variable. The analysis utilizes all previously considered samples. The ATT is then recomputed with the function `estimate_all`, conditioning only on the remaining set of covariates.
```{r, warning=FALSE}
#| code-fold: show
# define variables
Y_pl <- "nchildren75"
...
out3_pl <- estimate_all(lcs_psid.trim_match, Y_pl, "treat", covar_pl)
```
...
#| code-fold: show
# save results 
save_csv(result_mat_pl, "LDW_att_estimates_pl")
```

## Sensitivity Analyses

Finally, a sensitivity analyses using the LCS data is conducted and results are depicted in contour plots below using the function `sens_ana` of Imbens & Xu (2024).
```{r, warning=FALSE}
#| code-fold: show
# define variables
Y <- "re79"

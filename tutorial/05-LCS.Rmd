...
The tabulated results confirm visual patterns: Column (A) reports the estimates for the LCS-Experimental sample, column (B) for the PSID1 sample, and column (C) for the trimmed LCS-PSID1 sample. Columns (D)-(H) show results of the top-ranked sample of LCS-PSID1.
...
Specifically, with LCS-PSID1, CATT estimates span from $-3,357.48 to $2,404.33, contrasting with the CATT estimated from experimental data which ranges from $-359.43 to $2,719.14, with a mean CATT of $863.51. 

Across the PSID1-based subsamples, the ranges of CATT estimates are relatively similar. Importantly, the mean CATT estimates remain positive in all cases, except for the sample constructed with the `card` method, that produces a mean CATT estimate of $-415.71.
...
These figures present QTET estimates obtained from LCS-Experimental sample and several observational samples. The QTETs estimated from both the original and trimmed LCS-PSID1 samples (B and C), as well as from the top-ranked samples (E through H), align closely with the true QTET. By contrast, the QTETs from sample (F) exhibit a stronger bias, suggesting greater estimation uncertainty.
...
The placebo analysis shows that the experimental benchmark (first row, first column) for 1975 earnings is close to zero and statistically insignificant, while all estimators applied to the observational datasets produce comparable results.
```{r}
#| code-fold: show
# save results 

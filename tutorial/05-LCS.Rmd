...
for (wcol in weight_columns) {
  if (wcol %in% names(lcs_psid.percent)) {
    lcs_psid.percent <- truncate_weights_percentile(lcs_psid.percent, weight_col = wcol, lower = 0.05, upper = 0.95)
  }
}
```
...
for (wcol in weight_columns) {
  if (wcol %in% names(lcs_psid.adapt)) {
    lcs_psid.adapt <- truncate_weights_adaptive(lcs_psid.adapt, weight_col = wcol, treat_col = "treat")
  }
}
```

...
save_top5_individual_files(dataset_list_psid, top5_methods.psid, prefix = "lcs_psid1")
```
...
out3 <- estimate_all(lcs_psid_plus.trim, "re79", "treat", covar) # similar to Imbens & Xu (2024)
out4 <- estimate_all(lcs_psid.trim, "re79", "treat", covar)  
...
```{r, message=FALSE, warning=FALSE}
# build plot titles 
base_titles <- c("(A) LCS-Experimental", "(B) LCS-PSID1" , "(C) Trimmed LCS-PSID1-PLUS", "(D) Trimmed LCS-PSID1")
...
knitr::kable(result_mat, booktabs = TRUE, caption = "ATT Estimates and SEs") %>% kable_styling(full_width = TRUE) %>% scroll_box(width = "600px", height = "400px")
```
...
out3_pl <- estimate_all(lcs_psid_plus.trim, Y_pl, "treat", covar_pl) # similar to Imbens & Xu (2024)
out4_pl <- estimate_all(lcs_psid.trim, Y_pl, "treat", covar_pl)
```

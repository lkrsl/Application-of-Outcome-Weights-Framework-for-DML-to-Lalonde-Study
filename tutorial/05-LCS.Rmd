...
m.out.psid_plus.profile <- matchit(model, data = lcs_psid_plus, method = "cardinality", estimand = "ATT", tols = 0.1, ratio = NA, solver = "highs")
```
...
lcs_psid_plus$ipw_weight <- create_iptw_weights(lcs_psid_plus, treat, covar)
```
...
lcs_psid_plus$smr_weight <- create_smr_weights(lcs_psid_plus, treat, covar, "ATT")
```
...
lcs_psid_plus$overlap_weight <- create_overlap_weights(lcs_psid_plus, treat, covar)
```
...
lcs_psid_plus$ebal_weight <- create_entropy_weights(lcs_psid_plus, treat, covar)
```
...
for (wcol in weight_columns) {
  if (wcol %in% names(lcs_psid_plus.fixed)) {
    lcs_psid_plus.fixed <- truncate_weights_fixed(lcs_psid_plus.fixed, weight_col = wcol, lower = 0.025, upper = 0.975)
  }
}
```
...
for (wcol in weight_columns) {
  if (wcol %in% names(lcs_psid_plus.percent)) {
    lcs_psid_plus.percent <- truncate_weights_percentile(lcs_psid_plus.percent, weight_col = wcol, lower = 0.01, upper = 0.99)
  }
}
```

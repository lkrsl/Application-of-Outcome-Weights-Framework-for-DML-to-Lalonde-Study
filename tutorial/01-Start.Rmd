...
**Packages**: "cobalt", "data.table", "dplyr", "ebal", "ggplot2", "gridExtra", "highr", "highs" , "MatchIt", "optmatch", "optweight", "quickmatch", "readr", "rgenoud", "tidyr", "tidyverse", "WeightIt"
```{r, message=FALSE, warning=FALSE}
# required packages
packages <- c("cobalt", "data.table", "dplyr", "ebal", "ggplot2", "gridExtra", "highr",  "kableExtra", "MatchIt", "optmatch", "optweight", "quickmatch", "readr", "rgenoud", "tidyr", "tidyverse", "WeightIt"
)
...
plot_matching_balance <- function(matchit_objects, threshold = 0.1, title = NULL) {
  ...
  if (n_plots == 1) {
    ...
    for (i in seq(1, n_plots, by = plots_per_page)) {
      gridExtra::grid.arrange(grobs = plot_list[i:min(i + plots_per_page - 1, n_plots)],
                              ncol = 3, nrow = 2)
    }
  }
}
...
- `treat`: The name of the binary treatment indicator variable as string.
- `covar`: A vector of covariate names.
- `orig_cps`: The original unweighted CPS1 dataset prior to any weighting or trimming.
- `orig_psid`: The original unweighted PSID1 dataset prior to any weighting or trimming.
- `prefix_cps`: A string prefix for plot titles related to CPS results.
- `prefix_psid`: A string prefix for plot titles related to PSID results.
- `threshold`: Numeric value defining the acceptable standardized mean difference limit for covariate balance (default is 0.1).

...
| `ps_trim()`, `common_range_trim()`, `crump_trim()`, `stuermer_trim ()`, `walker_trim()`, `ps_estimate()`, `attach_matchit()` | Implements methods to improve covariate balancing and overlap and reestimate propensity scores.   | [More](#impl-cov-bal-overl-mthds)  |
| `compute_abs_smd_trim()`, `compute_ovl_trim()`, `compute_abs_smd_matchit()`, `compute_ovl_matchit()`, `combine_results()`, `save_csv()`, `assess_methods()`, `get_top_methods()`, `rerank_methods()`, `wrap_match_entries()`, `create_top5_datasets()`, `save_top5_datasets()` | Evaluates performance of previous methods through comparative metrics (absolute standardized mean differences (SMD) and overlapping coefficient (OVL)).    |
[More](#eval-mthds) |
...
- `data_list`: A list of data frames containing the subsetted dataset after trimming procedures.
...
### Assessing methods
#### Trimming
`compute_abs_smd_trim()` computes absolute SMDs for multiple datasets.
```{r}
...
`compute_ovl_trim()` compute the OVL for multiple datasets.
```{r}
...
- `data_list`: A list of data frames, each a trimmed dataset.
- `treat`: The name of the binary treatment indicator variable as string.
- `n_points`: The number of grid points to use for the kernel density estimation.

#### Integrated methods
`compute_abs_smd_matchit()` computes the absolute SMD for matchit objects.
```{r}
...
`combine_results()` merges and summarizes SMD and OVL statistics from various analysis stages of non-plus-datasets into a final comparison table.
```{r}
...
- `top_methods`: A character vector of method names for which to re-rank results.
- `ranked_table`: A data frame of method performance metrics.
...
- `top_method_names`: A character vector of method names corresponding to the top-ranked datasets. 
...
- `method_names`: A list of methods names that were used to generate the datasets.
- `balance_var`: The name of the covariate that shall be used for balance check.
- `folder`: A string specifying the folder path where the PDF panels will be saved (default is "graphs/lalonde").
- `plots_per_page`: The number of plots to display per page / panel (default 5).
- `prefix`: A string prefix for saving output PDF files.

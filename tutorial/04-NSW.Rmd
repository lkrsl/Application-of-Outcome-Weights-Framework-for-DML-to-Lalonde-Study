...
m.out.cps_plus.profile <- matchit(model, data = nsw_cps_plus, method = "cardinality", estimand = "ATT", tols = 0.1, ratio = NA, solver = "highs")
m.out.psid_plus.profile <- matchit(model, data = nsw_psid_plus, method = "cardinality", estimand = "ATT", tols = 0.1, ratio = NA, solver = "highs")
```
...
nsw_cps_plus$ipw_weight <- create_iptw_weights(nsw_cps_plus, treat, covar)
nsw_psid_plus$ipw_weight <- create_iptw_weights(nsw_psid_plus, treat, covar)
```
...
nsw_cps_plus$smr_weight <- create_smr_weights(nsw_cps_plus, treat, covar, "ATT")
nsw_psid_plus$smr_weight <- create_smr_weights(nsw_psid_plus, treat, covar, "ATT")
```

#### Matching weights
```{r, message=FALSE, warning=FALSE}
# calculate matching weights by assigning weight = min(PS, 1-PS)/PS to treated units and min(PS, 1-PS)/(1-PS) to controls
nsw_cps_plus$opt_weight <- create_matching_weights(nsw_cps_plus, treat, covar)
nsw_psid_plus$opt_weight <- create_matching_weights(nsw_psid_plus, treat, covar)
```
...
nsw_cps_plus$overlap_weight <- create_overlap_weights(nsw_cps_plus, treat, covar)
nsw_psid_plus$overlap_weight <- create_overlap_weights(nsw_psid_plus, treat, covar)
```
...
nsw_cps_plus$ebal_weight <- create_entropy_weights(nsw_cps_plus, treat, covar)
nsw_psid_plus$ebal_weight <- create_entropy_weights(nsw_psid_plus, treat, covar)
```
...
for (wcol in weight_columns) {
  if (wcol %in% names(nsw_cps_plus.fixed)) {
    nsw_cps_plus.fixed <- truncate_weights_fixed(nsw_cps_plus.fixed, weight_col = wcol, lower = 0.025, upper = 0.975) 
  }
}
...
for (wcol in weight_columns) {
  if (wcol %in% names(nsw_psid_plus.fixed)) {
    nsw_psid_plus.fixed <- truncate_weights_fixed(nsw_psid_plus.fixed, weight_col = wcol, lower = 0.025, upper = 0.975)
  }
}
```
...
for (wcol in weight_columns) {
  if (wcol %in% names(nsw_cps_plus.percent)) {
    nsw_cps_plus.percent <- truncate_weights_percentile(nsw_cps_plus.percent, weight_col = wcol, lower = 0.01, upper = 0.99)
  }
}
...
for (wcol in weight_columns) {
  if (wcol %in% names(nsw_psid_plus.percent)) {
    nsw_psid_plus.percent <- truncate_weights_percentile(nsw_psid_plus.percent, weight_col = wcol, lower = 0.01, upper = 0.99)
  }
}
```

...
The above figures show the ATT estimates and their 95% confidence intervals for fifteen samples: NSW-Experimental, NSW-CPS1, NSW-PSID1, trimmed versions of the NSW-CPS1 and NSW-PSID1 samples (analogous to Imbens & Xu) and a series of top-ranked subsamples of both NSW-CPS1 and NSW-PSID1 based on various matching, weighting, truncation and trimming criteria.

Figure (A) presents the benchmark from the experimental data (NSW-Experimental), serving as a reference for bias and variance assessment of observational samples. Figures (B) and (C) show results for the observational samples, NSW-CPS1 and NSW-PSID1, while figures (D) and (E) present those for the trimmed versions, replicating the tutorial results of Imbens & Xu (2024). Figures (F) through (J) display results for CPS1-derived subsamples constructed with the top-ranked methods. Analogously, figures (K) through (O) summarize results for the corresponding PSID1-derived subsamples under parallel rules.

Across the NSW-CPS1 and its top-ranked subsamples, all estimators consistently produce ATT estimates that closely align with the experimental benchmark, though all are negative. These estimates, however, deviate more from the benchmark than those obtained under model A and B in previous sections using LDW data. Notably, the `overlap_crump` and `adapt_weight_trunc_ipw_weight` methods exhibit somewhat larger deviations.

In comparison, PSID1-based subsamples frequently exhibit greater dispersion and substantially higher standard errors than NSW-CPS1 samples, mirroring observations from the LDW data. All ATT estimates remain negatively aligned, reflecting heightened methodological uncertainty within these samples.
```{r, warning=FALSE}
...
The tabulated results confirm visual patterns: Column (A) reports the estimates for the NSW-Experimental sample, column (B) for the NSW-CPS1 sample, and column (C) for the NSW-PSID1 sample. Columns (D)-(O) summarize the trimmed and top-ranked sample results for both NSW-CPS1 and NSW-PSID1.
...
Overall, consistent with findings from models A and B, figures and table jointly demonstrate that ATT estimates from observational samples tend to have larger standard errors compared to the experimental sample, reflecting greater statistical uncertainty in non-experimental causal effect estimation, and that trimming can bring observational estimates closer to the experimental benchmark. Nevertheless, significant estimator-dependent variability and sensitivity to sample construction persist.
```{r}
# save results 
...
Specifically, with NSW-CPS1, the CATT estimates range from $-8,652.64 to $6,452.97, contrasting with the experimental data where CATT estimates span from $-1,179.03 to $3,339.96, with a mean CATT estimate of $812.17. Unlike in the previous sections, the NSW-PSID1 data exhibits a narrower CATT estimate range, spanning from $-8,591.86 to $1,170.55 - yet its mean CATT estimate remains substantially negative, at approximately $-2,321.86.

Among the trimmed and top-ranked CPS1 subsamples, CATT estimates exhibit substantial variability. 
Methods such as `overlap_crump` or `adap_weight_trunc_ipw_weight` yield notably negative minimum CATT values alongside moderately negative mean estimates. Similarly, the `card` and `mahvars` methods produce consistently negative mean CATT estimates, indicating weaker alignment with the experimental benchmark. The `ps_threshold` sample performs better with highest min CATT and mean CATTs closest to the experimental benchmark, while still negative.

The CATT estimates for the PSID1 trimmed and top-ranked subsamples reveal substantially decreased mean CATT estimation values and wider extremes compared to their CPS1 counterparts, reflecting greater difficulties in producing reliable treatment effect estimates.

This variation in range and averages across methods and samples, as observed in previous sections,  reflects substantial heterogeneity in treatment effect estimation but also indicates that while certain samples or methods improve alignment with the experimental benchmark, others introduce considerable discrepancies and spread in estimated heterogeneous effects.
...
These figures display QTET estimates derived from both the NSW experimental and various observational samples. The QTETs estimated from the original and trimmed NSW-CPS1 sample (B and D) corresponds well with the true QTET, although the estimates are often underpowered. The QTET estimates from the original and trimmed NSW-PSID1 sample (C and E) show clear biases when compared to the experimental benchmark, which clusters near zero. Among the top-ranked subsamples, CPS1-based QTETs (F–J) consistently track the true QTET closely. In contrast, PSID1-based QTETs (K–O) generally perform well and align closely with the true QTET, except for subsamples based on the `mahvars` and `card` methods, which exhibit increased bias and noticeably wider confidence intervals, reflecting greater estimation uncertainty.
```{r, warning=FALSE}
# list results
plots_nsw <- list(
...
Consistent with the preceding sections, the evaluation shows that, across each dataset, the estimated outcome weights sum to one within the treated group and to minus one within the untreated group, yielding an overall total of zero.

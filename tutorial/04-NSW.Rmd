...
m.out.psids.optimal_pair <- matchit(model, data = nsw_psid, method = "optimal", distance = "logit")
```
...
m.out.cps_plus.cem <- matchit(model, data = nsw_cps, method = "cem")
m.out.psid_plus.cem <- matchit(model, data = nsw_psid, method = "cem")
```
...
nsw_psid$ipw_weight <- w.out.cps.ipw$weights
```
...
nsw_cps_plus.trim.match <- subset(nsw_cps_trim, sample %in% c(0,3) & ps_assoverlap)
nsw_psid_plus.trim.match <- subset(nsw_psid_trim, sample %in% c(0,4) & ps_assoverlap)
...
plot_matchit(all_match.cps, "NSW-CPS1")
plot_matchit(all_match.psid, "NSW-PSID1")
```
...
# list all weights
all_weight.cps <- list(
  ipw = nsw_cps$ipw_weight,
  opt = nsw_cps$opt_weight,
  cbps = nsw_cps$cbps_weight,
  ebal = nsw_cps$ebal_weight
)

all_weight.psid <- list(
  ipw = nsw_psid$ipw_weight,
  opt = nsw_psid$opt_weight,
  cbps = nsw_psid$cbps_weight,
  ebal = nsw_psid$ebal_weight
)
```
...
```{r, fig.cap='FIGURE C4.'}
# visualize covariate balance
plot_weighting_methods(nsw_cps, treat, covar, all_weight.cps, "NSW-CPS1") 
plot_weighting_methods(nsw_psid, treat, covar, all_weight.psid, "NSW-PSID1")
```
...
```{r, fig.cap='FIGUREC5.'}
# visualize covariate balance
plot_trunc_methods(all_trunc.cps, "treat", covar, weight_columns, "NSW-CPS1")
plot_trunc_methods(all_trunc.psid, "treat", covar, weight_columns, "NSW-PSID1")
```
...
```{r, fig.cap='FIGUREC6.'}
# visualize overlap
plot_trim(all_trim.cps, treat, covar)
plot_trim(all_trim.psid, treat, covar)
```
...
# visualize covariate balance
plot_comb_balance(comb_meth_cps = comb.cps, comb_meth_psid = comb.psid, treat = "treat", covar = covar, orig_cps = ldw_cps, orig_psid = ldw_psid, prefix_cps = "NSW-CPS1", prefix_psid = "NSW-PSID1")
```
...
```{r, message=FALSE, warning=FALSE}
# save results 
save_csv(top5_methods.cps_plus, "nsw_cps1_top5_methods")
save_csv(top5_methods.psid_plus, "nsw_psid1_top5_methods")
```

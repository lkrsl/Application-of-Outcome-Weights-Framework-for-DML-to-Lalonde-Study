...
### Overlap
```{r, class.source = "fold-show", fig.cap='FIGURE B1. SubfigureA:LDW. SubfigureB:LDW-CPS1. SubfigureC:LDW-PSID1.',out.width='100%', fig.asp=0.5}
#| code-fold: show
# assess overlap
...
```{r, class.source = "fold-show", fig.cap='FIGURE B1. SubfigureD:LDW-CPS1-PLUS. SubfigureE:LDW-PSID1-PLUS.', out.width='80%', fig.asp=1, fig.align='center'}
#| code-fold: show
# assess overlap
...
### Covariate balance
```{r, class.source = "fold-show", fig.cap='FIGURE B2. SubfigureA:LDW-CPS1. SubfigureB:LDW-CPS1-PLUS. SubfigureC:LDW-PSID1. SubfigureD:LDW-PSID1-PLUS.', out.width='80%', fig.asp=1, fig.align='center'}
...
#### Visuals
```{r, class.source = "fold-show", fig.cap='FIGURE B3.'}
#| code-fold: show
...
#### Visuals
```{r, class.source = "fold-show", fig.cap='FIGURE B4.'}
#| code-fold: show
# visualize covariate balance
...
#### Visuals
```{r, class.source = "fold-show", fig.cap='FIGURE B5.'}
#| code-fold: show
# visualize covariate balance
...
#### Visuals
```{r, class.source = "fold-show", fig.cap='FIGURE B6.'}
#| code-fold: show
# visualize overlap
...
```{r, class.source = "fold-show", fig.cap='FIGURE B7.'}
#| code-fold: show
# visualize covariate balance
...
#### Visuals
```{r, message=FALSE, warning=FALSE, class.source = "fold-show", fig.cap='FIGURE B8.'}
# visualize overlap
plot_comb_overlap(comb_meth.cps_plus, comb_meth.psid_plus, "treat", covar, prefix_cps = "LDW-CPS1", prefix_psid = "LDW-PSID1")
```

```{r, message=FALSE, warning=FALSE, class.source = "fold-show", fig.cap='FIGURE B9.'} 
# visualize covariate balance
...
Again, figure (A) presents the benchmark from the experimental sample (LDW-Experimental), serving as a reference for bias and variance assessment of observational results. Figures (B) and (C) show results for the observational samples, LDW-CPS1 and LDW-PSID1, respectively, while figures (D) and (E) present those for the trimmed versions. Figures (F) through (J) display results for CPS1-derived subsamples constructed with the top-ranked methods. Analogously, figures (K) through (O) summarize results for the corresponding PSID1-derived subsamples under parallel rules.

For LDW-CPS1, the top-ranked samples are based on the same methods as in model A. However, for the LDW-PSID1, the top-ranked samples are based on two different methods, `fix_max_value_trunc_overlap_weight` and `overlap_weight`.
...
For LDW-PSID1 and its subsamples, the estimates continue to exhibit greater dispersion and considerably larger standard errors compared to LDW-CPS1 sample, but closer to the benchmark than in model A. 
```{r, warning=FALSE}
...
In model B, using LDW-CPS1, the CATT estimates range from $-3,868.54 to $7,128.42. This span is considerably narrower than in model A, but still performs worse when contrasted with the experimental benchmark, where CATT estimates range from $-213.93 to $3,858.52 with a mean of $1,720.13. The experimental mean CATT remains closer to its corresponding ATT estimate, as already observed for model A. By contrast, LDW-PSID1 shows a narrower CATT range, from $-6,772.16 to	$5,032.30, compared to model A, with a mean CATT estimate of about $557.39 which is closer to the experimental benchmark, indicating better performance.
...
Top-ranked PSID1 subsamples, on the other hand, again yield more negative mean CATT estimates and wider ranges compared to their CPS1 counterparts, signaling persistent difficulties in generating reliable treatment effect estimates. Still, compared to model A, the minimum CATT estimates are less strongly negative, though the mean CATTs only improve marginally in their proximity to the experimental benchmark.

Hence, alike model A, these results show that the variation in ranges and averages across methods and samples reflects substantial heterogeneity in treatment effect estimation. While subsampling and trimming strategies can sometimes improve consistency with experimental benchmarks, they may also introduce notable discrepancies and variability in estimated heterogeneous effects. 
...
In model B, the evaluation once again confirms that, across each dataset, the estimated outcome weights sum to one within the treated group and to minus one within the untreated group, yielding an overall total of zero.
...
The placebo analysis shows that the experimental benchmark estimates remain close to zero and statistically insignificant, closely mirroring the results in model A. The results for LDW-CPS1 and LDW-PSID1 are identical to those observed in model A.

For LDW-CPS1 samples (B and F through J), ATT estimates remain consistently negative. The `overlap_crump` estimates show modest improvement toward the benchmark. Further gains are observed with the trimmed sample (D), `ps_threshold` (I), `card` (G) and `optimal_pair`. The `mahvars` (G) sample performs best, producing ATT estimates most consistent with the experimental results. However, compared to model A, no clear pattern emerges: some methods perform better, while others produce less accurate estimates.

For LDW-PSID1, the sample (O) applying `fix_max_value_truncation_overlap_weight` shows improvement relative to the PSID1 sample (C) — unlike in model A. The `overlap_weight` sample also shows some improvement, while the `card`, `mahvars`, trimmed sample (E) and the`ps_threshold` further reduce discrepancies, drawing estimates closer to the experimental benchmark results. Still, their performance not markedly surpass that of model A. 
...
```{r, out.width='100%', fig.asp=1, fig.cap="FIGURE 15. Sensitivity Analyses Model B"}
#| code-fold: show
